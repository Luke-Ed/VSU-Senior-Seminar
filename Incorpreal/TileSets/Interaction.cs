...
namespace Incorpreal.TileSets {
  public class Interaction : CanvasLayer {
    ...
    private string _actionState = "off";

    //The loot_areas to be disabled after the chest opens
    Godot.Collections.Array<Area2D> _lootAreas = new Godot.Collections.Array<Area2D>();
      ...
      if(Input.IsActionJustPressed("interact")) {
        ...
        switch (_actionState) {
          ...
          case "on":
            ...
            CollisionShape2D disableChest = (CollisionShape2D) (GetNode(((String) (_lootAreas[current_chest].GetPath() + "/CollisionShape2D"))));
            disableChest.Disabled = true;
    ...
    public void OnLootAreaEntered(Area2D area) {
      _actionState = "on";
      tile = (Vector2)usedTiles[current_chest]; 
      //Pass the current tile ingame to the process method
      tile_region = (Vector2)usedTextures[current_chest] + open_chest; 
      //Pass the Vector2 and change the texture to the "open chest" texture
      // GD.Print("Looting \n");
    }
